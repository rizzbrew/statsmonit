...
function initCharts() {
  ...
  cpuChart = new Chart(document.getElementById("cpu-chart").getContext("2d"), {
    ...
    data: {
      ...config.data,
      datasets: [
        { ...config.data.datasets[0], backgroundColor: ["#3B82F6", "#1F2937"] },
      ],
    },
  });
  ...
  ramChart = new Chart(document.getElementById("ram-chart").getContext("2d"), {
    ...
    data: {
      ...config.data,
      datasets: [
        { ...config.data.datasets[0], backgroundColor: ["#10B981", "#1F2937"] },
      ],
    },
  });
...
function updateChart(chart, value) {
  ...
  chart.data.datasets[0].backgroundColor[0] = isOverload
    ? "#EF4444"
    : chart.originalColor;
  chart.update();
}
...
function updateStats(data) {
  ...
  document.getElementById("load-15").textContent =
    data.load_average[2].toFixed(2);
...
socket.on("disconnect", () => {
  ...
  document
    .querySelectorAll(".chart-container, span, p")
    .forEach((el) => el.classList.add("animate-pulse"));
});
